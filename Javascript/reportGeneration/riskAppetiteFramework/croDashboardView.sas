...
%macro generate_cro_dashboard_view;
    ...
    put '    html += ''<div class="trend-chart-wrap"><svg width="560" height="210" viewBox="0 0 560 210" role="img" aria-label="Trend of watch/breach indicator counts by pillar">'';';
    put '    [0, 0.25, 0.5, 0.75, 1].forEach(f => {';
    put '      const y = bottom - f * (bottom - top);';
    put '      html += ''<line x1="'' + left + ''" y1="'' + y.toFixed(1) + ''" x2="'' + right + ''" y2="'' + y.toFixed(1) + ''" stroke="#dbe3f0" stroke-width="1"/>'';';
    put '      html += ''<text x="'' + (left - 6) + ''" y="'' + (y + 4).toFixed(1) + ''" text-anchor="end" font-size="11" fill="#66718a">'' + Math.round(f * maxVal) + ''</text>'';';
    put '    });';
    put '    html += ''<polyline points="'' + pointsFor(p1) + ''" fill="none" stroke="#2f7fd6" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>'';';
    put '    html += ''<polyline points="'' + pointsFor(p2) + ''" fill="none" stroke="#5b4fc4" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>'';';
    put '    periods.forEach((d, i) => {';
    put '      html += ''<circle cx="'' + xAt(i).toFixed(1) + ''" cy="'' + yAt(p1[i]).toFixed(1) + ''" r="4" fill="#2f7fd6" stroke="#fff" stroke-width="2"><title>Pillar 1 - '' + RAFUtils.formatDate(d) + '': '' + p1[i] + ''</title></circle>'';';
    put '      html += ''<circle cx="'' + xAt(i).toFixed(1) + ''" cy="'' + yAt(p2[i]).toFixed(1) + ''" r="4" fill="#5b4fc4" stroke="#fff" stroke-width="2"><title>Pillar 2 - '' + RAFUtils.formatDate(d) + '': '' + p2[i] + ''</title></circle>'';';
    put '      html += ''<text x="'' + xAt(i).toFixed(1) + ''" y="198" text-anchor="middle" font-size="11" fill="#4c5b76">'' + RAFUtils.formatDate(d) + ''</text>'';';
    put '    });';
    put '    html += "</svg></div>";';
    put '    return html;';
    put '  }';
